...
export const weixinPlugin: ChannelPlugin<ResolvedWeixinAccount> = {
  ...
  meta: {
    id: WEIXIN_CHANNEL_ID,
    label: WEIXIN_CHANNEL_ID,
    selectionLabel: `${WEIXIN_CHANNEL_ID} (long-poll)`,
    docsPath: `/channels/${WEIXIN_CHANNEL_ID}`,
    docsLabel: WEIXIN_CHANNEL_ID,
  ...
  outbound: {
    ...
    sendMedia: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        throw new Error(
          `weixin not configured: please run \`openclaw channels login --channel ${WEIXIN_CHANNEL_ID}\``,
        );
      }
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      if (!account.configured) {
        ...
        ctx.log?.error?.(
          `[${account.accountId}] weixin not logged in — run: openclaw channels login --channel ${WEIXIN_CHANNEL_ID}`,
        );
