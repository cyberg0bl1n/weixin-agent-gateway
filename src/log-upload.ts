...
function getConfiguredUploadUrl(config: OpenClawConfig): string | undefined {
  const section =
    (config.channels?.[WEIXIN_CHANNEL_ID] as { logUploadUrl?: string } | undefined) ??
    (config.channels?.[LEGACY_WEIXIN_CHANNEL_ID] as { logUploadUrl?: string } | undefined);
  return section?.logUploadUrl;
}

/** Register the Weixin channel utility CLI subcommand. */
export function registerWeixinCli(params: { program: CliCommand; config: OpenClawConfig }): void {
  ...
  root
    ...
    .action(async (options: { url?: string; file?: string }) => {
      ...
      if (!uploadUrl) {
        console.error(
          `[weixin] No upload URL specified. Pass --url or set it with:\n  openclaw config set channels.${WEIXIN_CHANNEL_ID}.logUploadUrl <url>`,
        );
        process.exit(1);
      }
